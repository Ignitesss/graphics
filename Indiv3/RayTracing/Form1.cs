...
namespace RayTracing
{
    public partial class Form1 : Form
    {
        ...
        public XYZPoint[,] pixels;
        public XYZPoint focus;
        public XYZPoint up_left, up_right, down_left, down_right;
        public int h, w;
        ...
        public Form1()
        {
            InitializeComponent();
            focus = new XYZPoint();
            up_left = new XYZPoint();
            up_right = new XYZPoint();
            down_left = new XYZPoint();
            down_right = new XYZPoint();

            h = pictureBox1.Height;
            w = pictureBox1.Width;
            pictureBox1.Image = new Bitmap(w, h);

            cubeSpecularCB.Checked = sphereSpecularCB.Checked = refractCubeCB.Checked = refractSphereCB.Checked = false;
        ...
        public void build_scene()
        {
            ...
            XYZPoint normal = Side.norm(room.sides[0]);                            // нормаль стороны комнаты
            XYZPoint center = (up_left + up_right + down_left + down_right) / 4;   // центр стороны комнаты
            ...
            Light l1 = new Light(new XYZPoint(0f, 1f, 4.9f), new XYZPoint(1f, 1f, 1f));
            lights.Add(l1);
            if (twoLightsCB.Checked)
            {
                Light l2 = new Light(new XYZPoint(0f, 4f, -4.9f), new XYZPoint(1f, 1f, 1f));
                lights.Add(l2);
            }

            Sphere s1 = new Sphere(new XYZPoint(2.8f, -2.5f, 2.5f), 2f);
            ...
            Sphere s2 = new Sphere(new XYZPoint(-2.2f, 1.6f, -1.4f), 1.2f);
            s2.set_pen(new Pen(Color.DarkOrange));
            s2.figure_material = new Material(0.0f, 0.0f, 0.1f, 0.7f, 1.0f);

            Sphere s3 = new Sphere(new XYZPoint(2.5f, 2f, -3.4f), 1.7f);
            ...
            Sphere s4 = new Sphere(new XYZPoint(2.7f, -2.5f, 3.95f), 1.3f);
        ...
        public void run_rayTrace()
        {
            ...
            for(int i = 0; i < w; ++i)
                 for(int j = 0; j < h; ++j)
                {
                    ...
                    r.start = new XYZPoint(pixels[i, j]);
                    XYZPoint clr = RayTrace(r, 10, 1);
                    if (clr.x > 1.0f || clr.y > 1.0f || clr.z > 1.0f)
                        clr = XYZPoint.norm(clr);
        ...
        {
            pixels = new XYZPoint[w, h];
            color_pixels = new Color[w, h];
            XYZPoint step_up = (up_right - up_left) / (w - 1);
            XYZPoint step_down = (down_right - down_left) / (w - 1);

            XYZPoint up = new XYZPoint(up_left);
            XYZPoint down = new XYZPoint(down_left);

            for (int i = 0; i < w; ++i)
            {
                XYZPoint step_y = (up - down) / (h - 1);
                XYZPoint d = new XYZPoint(down);
                for (int j = 0; j < h; ++j)
                {
                    pixels[i, j] = d;
                    d += step_y;
        ...
        public bool is_visible(XYZPoint light_point, XYZPoint hit_point)
        {
            ...
            foreach(Figure fig in scene)
                if (fig.figure_intersection(r, out float t, out XYZPoint n))
                    if (t < max_t && t > Figure.EPS)
                        return false;
             return true;
        }

        public XYZPoint RayTrace(Ray r, int iter, float env)
        {
            if (iter <= 0)
                return new XYZPoint(0, 0, 0);

            float t = 0;        // позиция точки пересечения луча с фигурой на луче
            XYZPoint normal = null;
            Material m = new Material();
            XYZPoint res_color = new XYZPoint(0, 0, 0);
            bool refract_out_of_figure = false; //  луч преломления выходит из объекта?

            foreach(Figure fig in scene)
            {
                if (fig.figure_intersection(r, out float intersect, out XYZPoint n))
            ...
            if (XYZPoint.scalar(r.direction, normal) > 0) 
            {
                normal *= -1; 
                refract_out_of_figure = true;
            }

            XYZPoint hit_point = r.start + r.direction * t;

            foreach(Light l in lights)
            {
                XYZPoint amb = l.color_light * m.ambient;
                amb.x = (amb.x * m.clr.x);
                amb.y = (amb.y * m.clr.y);
                amb.z = (amb.z * m.clr.z);
                res_color += amb;
