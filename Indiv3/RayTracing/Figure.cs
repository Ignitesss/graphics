...
namespace RayTracing
{
    public class Figure
    {
        ...
        public List<XYZPoint> points = new List<XYZPoint>(); // точки 
        public List<Side> sides = new List<Side>();        // стороны
        public Material figure_material;

        public bool isRoom = false;                       // комната
        ...
        {
            foreach (XYZPoint p in f.points)
                points.Add(new XYZPoint(p));
        ...
        public bool ray_intersects_triangle(Ray r, XYZPoint p0, XYZPoint p1, XYZPoint p2, out float intersect)
        {
            intersect = -1;

            XYZPoint edge1 = p1 - p0;
            XYZPoint edge2 = p2 - p0;
            XYZPoint h = r.direction * edge2;
            float a = XYZPoint.scalar(edge1, h);

            if (a > -EPS && a < EPS)
                return false;

            float f = 1.0f / a;
            XYZPoint s = r.start - p0;
            float u = f * XYZPoint.scalar(s, h);

            if (u < 0 || u > 1)
                return false;

            XYZPoint q = s * edge1;
            float v = f * XYZPoint.scalar(r.direction, q);

            if (v < 0 || u + v > 1)
                return false;
            float t = f * XYZPoint.scalar(edge2, q);
            if (t > EPS)
            {
                intersect = t;
                return true;
        ...
        public virtual bool figure_intersection(Ray r, out float intersect, out XYZPoint normal)
        {
            intersect = 0;
            normal = null;
            Side sd = null;
            ...
            if (intersect != 0)
            {
                ...
                figure_material.clr = new XYZPoint(sd.drawing_pen.Color.R / 255f, sd.drawing_pen.Color.G / 255f, sd.drawing_pen.Color.B / 255f);
                return true;
            }
        ...
        public void line_rotate_rad(float rang, XYZPoint p1, XYZPoint p2)
        {

            p2 = new XYZPoint(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
            p2 = XYZPoint.norm(p2);
        ...
        public void line_rotate(float ang, XYZPoint p1, XYZPoint p2)
        {
            ang = ang * (float)Math.PI / 180;
            line_rotate_rad(ang, p1, p2);
        }

        private static float[,] rotate_around_line(float[,] transform_matrix, XYZPoint start, XYZPoint dir, float angle)
        {
        ...
        static public Figure get_Hexahedron(float sz)
        {
            ...
            res.points.Add(new XYZPoint(sz / 2, sz / 2, sz / 2)); // 0 
            res.points.Add(new XYZPoint(-sz / 2, sz / 2, sz / 2)); // 1
            res.points.Add(new XYZPoint(-sz / 2, sz / 2, -sz / 2)); // 2
            res.points.Add(new XYZPoint(sz / 2, sz / 2, -sz / 2)); //3

            res.points.Add(new XYZPoint(sz / 2, -sz / 2, sz / 2)); // 4
            res.points.Add(new XYZPoint(-sz / 2, -sz / 2, sz / 2)); //5
            res.points.Add(new XYZPoint(-sz / 2, -sz / 2, -sz / 2)); // 6
            res.points.Add(new XYZPoint(sz / 2, -sz / 2, -sz / 2)); // 7
